...
const Menu = () => {
    ...
    const categoryItem = categories?.map(item => (
        <li
            ...
            className={`py-2 px-5 border rounded-md cursor-pointer ${`/category/${item}` === category ? "text-black border-[#DC780B] bg-white" : "bg-[#DC780B] text-white"}`}
            key={item}>
            {item}
        </li>
    ));
    ...
    const productsItem = data?.products?.map((product) => (
        ...
        data-aos-anchor-placement="center-bottom">
            <div className='h-[264px] w-full p-6 bg-white rounded-tl-[75px] rounded-tr-[10px] rounded-br-[75px] cursor-pointer'>
        ...
        <section className='mb-[50px]'>
            <div className="container__person">
                <div className='w-full text-center mb-[60px]'>
                    <p className="text-[#DC780B] uppercase font-medium tracking-widest">Menu</p>
                    <h2 className="Philosopher text-[32px] font-bold">Food Full of Treaty Love</h2>
                    <p className='max-w-[750px] text-center inline-block'>There are many things needed to start the Fast Food Business. You need not only Food Stalls with Staff but also specialized equipment, and skills to manage Customers.</p>
                </div>
                <ul className='flex whitespace-nowrap gap-3 overflow-auto pb-5'>
                    <li
                        className={`py-2 px-5 border rounded-md cursor-pointer ${`/` === category ? "text-black border-[#DC780B] bg-white" : "bg-[#DC780B] text-white"}`}
                        onClick={() => setCategory("/")}
                        key="All">
                        All
                    </li>
                ...
                <div className='w-full text-center mt-12'>
                    {offset * count < data?.total && (
                        <button
                            className='rounded-md py-[12px] px-[25px] bg-[#DC780B] text-white border border-[#DC780B] duration-500 hover:bg-white hover:text-black disabled:cursor-not-allowed'
...
//                     {loading ? <Loading /> : error ? <p>Error loading products.</p> : productsItem}
//                 </div>
//                 <div className='w-full text-center mt-12'>
//                     {offset * count < data?.total && (
//                         <button
//                             className='rounded-md py-[12px] px-[25px] bg-[#DC780B] text-white border border-[#DC780B] duration-500 hover:bg-white hover:text-black disabled:bg-orange-500 disabled:cursor-not-allowed'
//                             onClick={() => setOffset(p => p + 1)}
//                             disabled={loading}
//                         >
//                             {loading ? "Loading..." : "See More"}
